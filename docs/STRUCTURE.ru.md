...
### 1. Общие правила:
  ...
  ```
  3. Фигурные скобки {} всегда открываются на той же строке, закрываются на отдельной строке:
  ```
  class My_Class {
  ...
  }
  ...
  4. Между логическими блоками свойств допускается одна пустая строка.
  5. Вложенные маленькие классы (≤3 свойства) можно писать без пустых строк внутри.

### 2. Порядок секций в классе
  Рекомендуемый порядок секций внутри класса:
  1. Основные свойства
    - scope, author, displayName, side
  2. Модель и визуальные элементы
    - model, picture, hiddenSelectionsTextures
  3. Экипировка и оружие
    - weapons[], magazines[], linkedItems[]
  4. EventHandlers
    - init, fired, killed
